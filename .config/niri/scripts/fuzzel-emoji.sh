...
# Modes: type | copy | both
# Categories: emoji | ascii | ask (prompts with fuzzel)
ACTION="${1:-type}"
CATEGORY="${2:-ask}"

if [[ "$CATEGORY" == "ask" ]]; then
    CATEGORY=$(printf "Emoji\nKaomoji (ASCII)" | fuzzel --dmenu --prompt "Select collection: ")
    case "$CATEGORY" in
        "Emoji") CATEGORY="emoji" ;;
        "Kaomoji (ASCII)") CATEGORY="ascii" ;;
        *) exit 0 ;;
    esac
fi

# Extract the requested section using awk
get_data() {
    local target="$1"
    awk -v target="$target" '
        $0 ~ "^### DATA " target " ###$" { flag=1; next }
        /^### DATA/ { flag=0 }
        flag { print }
    ' "$0"
}
...
case "$CATEGORY" in
    ...
    ascii|kaomoji)
        ...
        ;;
    *)
        echo "Usage: $0 [type|copy|both] [emoji|ascii|ask]"
        exit 1
        ;;
esac

# fuzzel selection: split on tab (\t) so multi-word/spaced kaomoji remain intact
# fuzzel selection
selected_line="$(echo -n "$DATA" | fuzzel --match-mode fzf --dmenu --prompt "$PROMPT")"
...
case "$CATEGORY" in
    emoji)
        selected="$(awk '{print $1}' <<< "$selected_line")"
        ;;
    ascii|kaomoji)
        selected="$(sed -E 's/[[:space:]]{2,}.*//' <<< "$selected_line")"
        ;;
esac
...
### DATA ASCII ###
¯\_(ツ)_/¯      	shrug idk whatever don't know meh
¯\(°_o)/¯      	shrug confused dunno derp whatever
ಠ_ಠ            	disapproval look stare serious judge meh
ಠ‿ಠ            	creepy smile sinister plan scheming evil
( ͡° ͜ʖ ͡°)      	lenny face cheeky smirk pervy pervert suspicious
( ° ͜ʖ °)       	mild lenny subtle smirk smug
(づ｡◕‿‿◕｡)づ  	hug embrace cute cuddle wholesome love
(っ˘̩╭╮˘̩)っ  	hug comfort sad crying consoling
(づ￣ ³￣)づ    	blow kiss hug affection cute love
(づ｡◕‿◕｡)づ    	hug arms open cute sweet
(つ≧▽≦)つ      	hug excited happy embrace squeeze
⊂(･ω･*⊂)      	hug side squeeze cute gentle
(づ◡﹏◡)づ     	gentle comforting cuddle slow embrace
(・_・;)        	nervous sweat worried sweatdrop awkward
(;¬_¬)         	side eye suspicious doubtful skeptic squint
(¬_¬ )         	side eye looking away skepticism doubt
(¬_¬)          	unimpressed skeptical sigh side eye whatever
(¬‿¬)          	smug smirk sneaky playful sneaky
(¬_¬")         	irritated sigh annoyed side eye sweat
(눈_눈)         	dead eyes unamused judging stare silent
(⌐■_■)         	cool sunglasses deal with it boss swag
(ง'̀-'́)ง        	fight boxing ready hit brawl punch
(ง •̀_•́)ง      	determined fight motivated ganbatte cheer
(ง ื▿ ื)ว      	dance happy boogie groove celebration
(•̀ᴗ•́)و         	fist pump yes victory determination got this
(๑•̀ㅂ•́)و✧     	determined spark win success achieve cheer
ᕦ(ò_óˇ)ᕤ       	flexing arms strong muscles power angry
ᕦ( ͡° ͜ʖ ͡°)ᕤ   	flex lenny strong buff meme
(╥﹏╥)         	crying tears sob weeping sad bawling
(个_个)         	crying tears streaming waterfall sad weep
(ಥ﹏ಥ)         	tears sobbing holding back cry sad emotional
(つд⊂)         	hide face peek crying shy ashamed embarrassed
(T_T)          	crying classic tears sad sob weep
(ㅠ_ㅠ)         	crying tears streaming korean sad weep
(ノ_<。)       	crying wiping tears pain upset
